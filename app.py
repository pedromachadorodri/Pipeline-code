...
# Pedro - 130421 - início
        # if type(self.strong_binders) is int:
        #     self.strong_binders = [self.strong_binders]
        # else:
        #     self.strong_binders = [float(i) for i in self.strong_binders.split(',')]
        #
        # if type(self.weak_binders) is int:
        #     self.weak_binders = [self.weak_binders]
        # else:
        #     self.weak_binders = [float(i) for i in self.weak_binders.split(',')]
# Pedro - 130421 - termina
    ...
    def analysis(self):
        ...
        # strong_binders = self.strong_binders
        # weak_binders = self.weak_binders
        preds = self.preds
        ...
        for P in self.preds:
            ...
            pb.to_csv(os.path.join(self.path,'final_%s_n=%s.csv' %(p,n)), float_format='%g')
            # Pedro - adição - 100521 - início
            outputfas = os.path.join(self.path,'epitopes_%s.fasta'  %(p))
            fasep = sequtils.dataframe_to_fasta(pb, outfile=outputfas, seqkey='peptide', idkey='peptide')
            pb2 = P.promiscuous_binders2(binders=b, predictor=p,n=n, cutoff=cutoff, cutoff_method=cutoff_method)
            print ('found %s promiscuous binders at cutoff=%s, n=%s' %(len(pb2),cutoff,n))
            pb2.to_excel(os.path.join(self.path,'promiscuous_epitopes_with_alleles_%s.xlsx' %(p)))
            # Pedro - adição - 100521 - termino
            prom_binders[p] = pb
            if len(pb)>0:
...
# #Adicionou----------------------------------------------------
# def iedb_checks(method):
#     if check_iedbmhc2_path() == False:
#         return False
#     if check_iedb_method(method) == False:
#         return False
#     return True
#
# def check_mhc2_length(l):
#     if l<11 or l>30:
#         print ('use MHCII n-mer lengths from 11-30')
#         return False
#
