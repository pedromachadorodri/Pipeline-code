...
# Pedro - adição - def dataframe_withoutgap, list_of_peptides_from_dataframe - 140721
def dataframe_withoutgap (aln=None,fastafile=None):
    """Get dataframe from alignment or fasta file without gaps"""
    if fastafile is not None:
        recs = SeqIO.parse(fastafile,'fasta')
        resultado = [(i.name,str(i.seq.ungap("-")),str(i.description)) if '-' in i else (i.name,str(i.seq),str(i.description)) for i in recs]
        df = pd.DataFrame(resultado,columns=['name','seq','description'])
    else:
        resultado = [(i.name,str(i.seq.ungap("-")),str(i.description)) if '-' in i else (i.name,str(i.seq),str(i.description)) for i in aln]
        df = pd.DataFrame(resultado,columns=['name','seq','description'])
    
    return df    

def list_of_peptides_from_dataframe(dataframe,column='peptide'):
    "get a list o peptides from a dataframe"
    pep=[]
    for i,row in dataframe.iterrows():
    #     print(row.peptide)
        pep.append(row[column])
    # print(pep)
    # print(len(pep))
...
def fasta_format_from_feature(feature):
    ...
    return fastafmt
# Pedro - adição - descr - 06072021
def dataframe_to_seqrecords(df, seqkey='sequence', idkey='id',descr='description'):
    """dataframe to list of Bio.SeqRecord objects"""

    seqs=[]
    for i,r in df.iterrows():
        s=SeqRecord(Seq(r[seqkey]),id=r[idkey],description=r[idkey], name=r[idkey]) # Pedro - adição - description, name - 06072021
        seqs.append(s)
    return seqs
